...
export function buildActiveOrderStateBlock(input: ActiveOrderStateInput): string {
  ...
  lines.push(
    ...
    "اسأل عن حقل واحد فقط في الرد الواحد، بالترتيب المذكور، ولا تعرض للعميل أي من البيانات المسجَّلة هنا ولا تلخّصها له أثناء التجميع؛ البيانات تظهر مرة واحدة فقط في الملخص النهائي بعد اكتمال كل الحقول.",
  );
